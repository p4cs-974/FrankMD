...
function fed
  ...
  set -l running_id (docker ps -q -f name=frankmd 2>/dev/null)
  ...
  set running_id (docker ps -q -f name=frankmd 2>/dev/null)
  if test -z "$running_id"
    docker rm frankmd >/dev/null 2>&1
    ...
    for var in $env_vars
      set -l value (printenv "$var" 2>/dev/null)
      if test $status -eq 0
        set -a env_flags -e "$var=$value"
      end
    end
    ...
    if test -z "$images_dir"; and test -f "$notes/.fed"
      set images_dir (grep -m1 -E '^images_path[[:space:]]*=' "$notes/.fed" | sed "s/^[^=]*=[[:space:]]*//" | sed "s/^['\\\"]\\|['\\\"][[:space:]]*\$//g")
      set images_dir (string replace -r '^~' "$HOME" -- "$images_dir")
    end

    if test -z "$images_dir"
      if set -q XDG_PICTURES_DIR; and test -n "$XDG_PICTURES_DIR"
    ...
    set -l env_file_flags
    if set -q FRANKMD_ENV; and test -n "$FRANKMD_ENV"
      set env_file_flags --env-file "$FRANKMD_ENV"
    end
...
# Stop FrankMD
function fed-stop
  if docker stop frankmd >/dev/null 2>&1
    echo "Stopped."
  else
    echo "Not running."
  end
